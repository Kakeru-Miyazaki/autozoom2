...
labelDay = tkinter.Label(text="the Day (Sun, Mon, Tue, Wed, Thu, Fri, Sat)", fg="Black")  # , bg=bgcolor)
...
labelTime = tkinter.Label(
    text="the meeting time. (10-15 minute before the start is best.)", fg="Black")  # , bg=bgcolor)
...
labelName = tkinter.Label(text="the meeting name", fg="Black")  # , bg=bgcolor)
...
labelURL = tkinter.Label(text="the meeting URL", fg="Black")  # , bg=bgcolor)
...
pReg = "../icon/button/register.png"
pQuit = "../icon/button/quit.png"

imageReg = Image.open(pReg)
imageQuit = Image.open(pQuit)

imageReg = imageReg.resize((90, 35))
imageQuit = imageQuit.resize((90, 35))
# 720 280

imageReg = ImageTk.PhotoImage(imageReg)
imageQuit = ImageTk.PhotoImage(imageQuit)

button = tkinter.Button(root, text="register", command=register, bg=bgcolor, image=imageReg)
button.grid(column=0, row=9)
# button.pack()

buttonQuit = tkinter.Button(root, text="quit", command=quit, bg=bgcolor, image=imageQuit)
